...
void draw() {
  background(toGrayscale(170, 220, 240));
...
void drawFish(float x, float y) {
  ...
  // --- FISH BODY ---
  fill(toGrayscale(200, 100, 100));
  stroke(toGrayscale(150, 70, 70));
  strokeWeight(2);
  ellipse(x, y, 150, 80);
...
// NON-VOID FUNCTION: Returns a grayscale value
// Converts RGB to grayscale by averaging the three values
float toGrayscale(float r, float g, float b) {
  float gray = (r + g + b) / 3;
  return gray;  // Send the answer back!
}
