// Fish Demo - void functions with INTERACTIVITY (no conditionals)
// --- CUSTOM VARIABLES ---
float fish0X;
float fish0Y;
float speed0 = 2;
...
void mousePressed() {
  // Move fish0 to where you clicked
  fish0X = mouseX;
  fish0Y = mouseY;
}
...
void keyPressed() {
  // Reset all fish to starting positions
  fish0X = width / 2;
  fish0Y = height / 2;
  fish1X = 100;
  fish1Y = 100;
...
void displayInfo() {
  ...
  // Show controls
  fill(0, 0, 150);
  text("--- CONTROLS ---", 10, 200);
  text("Click/Drag = Move fish 0", 10, 215);
  text("Any key = Reset all fish", 10, 230);
}
